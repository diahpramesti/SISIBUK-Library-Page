...
function sparql_fetch_all($result) {
  $all_results = []; // Use an array to collect results
  while ($row = $result->fetch_array()) { // Fetch all rows
    $all_results[] = $row; // Add each row to the array
  }
  return $all_results; // Return the array
}

function sparql_get($endpoint, $sparql) {
  $db = sparql_connect($endpoint); // Connect to the endpoint
  if (!$db) { 
    return null; // Return null if connection fails
  }
  $result = $db->query($sparql); // Execute the query
  if (!$result) {
    return null; // Return null if the query fails
  }
  return sparql_fetch_all($result); // Fetch all results as an array
}
